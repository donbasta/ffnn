...
#             print("huhuhu", layer.delta)
        ...
        for i in range(num_layers):
            ...
            grad["dW" + str(i)] = (np.dot(layer.delta, input_activation.T) * self.learning_rate / len(y_train))
            dZ = (layer.delta * self.learning_rate)
            grad["db" + str(i)] = (np.sum(dZ, axis=1).reshape(len(dZ), 1))
